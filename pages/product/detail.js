...
class ProductSave extends Component{	
  ...
  handleSubmit(e){
    ...
    this.props.form.validateFields((err, values) => {
    	if(this.state.productId){
  		  this.props.handelUpdateSave(this.state.productId,err,values);
  	  }else{
...
const mapDisPatchToProps=(dispatch)=>{
  return{
    ...
    handelCategoryId:(parentCategoryId,caregoryId)=>{
      const action=actionCreator.categoryIdAction(parentCategoryId,caregoryId);
      dispatch(action);
    },
    handelImage:(fileList)=>{
      const action=actionCreator.ImageAction(fileList);
      dispatch(action);
    },
    ...
    handelUpdateSave:(productId,err,values)=>{
    	dispatch(actionCreator.UpdateSaveAction(productId,err,values))
    }
  }
}
